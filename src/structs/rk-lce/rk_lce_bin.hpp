...
public:
   ...
   *
   */
  inline bool operator[](uint64_t i) {
   ...
   *
   */
  inline uint128 operator()(uint64_t i, uint64_t len = 128) {
  ...
  inline uint64_t number_of_blocks() {
    return Q1.size();
  }

  inline uint64_t block_size() {
    return w;
  }
...
private:
   ...
   *
   */
  inline uint128 RK(uint64_t i, uint64_t j, uint128 rki = q) {
   ...
   *
   */
  inline uint64_t LCE_binary(uint64_t i, uint64_t j) {
  ...
  class suffix_comparator {
    ...
    public:
      ...
      inline sc_iterator& operator++() {
        ++_index;
        return *this;
      }

      // EqualityComparable
      inline bool operator==(sc_iterator it) const {
      ...
      inline sc_iterator operator++(int) {
        sc_iterator it(*this);
        ++_index;
        return it;
      }
      ...
      inline sc_iterator& operator+=(uint64_t n) {
        _index += n;
        return *this;
      }

      inline sc_iterator operator+(uint64_t n) const {
        sc_iterator it(*this);
        it += n;
        return it;
      }

      inline friend sc_iterator operator+(uint64_t n,
                                                      sc_iterator it) {
        return it + n;
      }
      ...
      inline uint64_t operator-(sc_iterator it) {
        return uint64_t(_index) - uint64_t(it._index);
      }

      inline bool operator[](uint64_t i) const {
  ...
  public:
    ...
    inline bool operator[](uint64_t t) {
    ...
    inline sc_iterator begin() {
      return sc_iterator(this, 0);
    }
    inline sc_iterator end() {
