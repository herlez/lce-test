...
class lcePrezzaAdv : public lceDataStructure {
	public:
		...
		lcePrezzaAdv(const std::string path) {
		...
		lcePrezzaAdv(const std::string path, const uint64_t numberOfChars) {
			std::ifstream input(path, std::ios::in|std::ios::binary);
			util::inputErrorHandling(&input);
		...
		uint64_t lce(const uint64_t i, const uint64_t j) {
			...
			int lce = 0;
			uint64_t iStartBlock = i/8;
			uint64_t jStartBlock = j/8; 
			
			uint64_t iBlock[9]; iBlock[8] = getBlock(iStartBlock++);
			uint64_t jBlock[9]; jBlock[8] = getBlock(jStartBlock++);
			
			
			/* Reverse the first block */
			uint64_t paquet = *(uint64_t*)"\x1\x0\x0\x0\x0\x0\x0\x0";
			if(paquet == 1){
				char * input1 = ((char*) iBlock) + 64;
				char * input2 = ((char*) jBlock) + 64;
				//reverse
				char *f1=&input1[0], *b1=&input1[7];
				char *f2=&input2[0], *b2=&input2[7];
				while(f1<b1){
					char tmp1 = *f1;
					char tmp2 = *f2;
					*f1++ = *b1;
			...
			for(int l = 0; l < 16; l++) {
				/* First we extract the blocks */
				iBlock[0] = iBlock[8];
				jBlock[0] = jBlock[8];
				for(int k = 1; k < 9; k++) {
					iBlock[k] = getBlock(iStartBlock++);
					jBlock[k] = getBlock(jStartBlock++);
				}
				
				/* Reverse the blocks, because they are in opposite order */
				char * input1 = (char*) iBlock;
				char * input2 = (char*) jBlock;
				...
				for(int blockLCE = 0; blockLCE <= 64; blockLCE++) {
					if (iCmp[blockLCE] != jCmp[blockLCE]) {
						return lce + blockLCE;
					}
				}
				lce += 64;
