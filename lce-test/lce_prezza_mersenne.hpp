...
public:
   ...
   */
  LcePrezzaMersenne(const std::string path, const uint64_t n) : 
					n_{n}, 
					text_{new char[n]} {
	...
	if(n > data_size) {
		std::cerr << "Attemted to load the first " << n << " Bytes, but the file " << path << " is only " << data_size << " Bytes big." << std::endl;
		exit(EXIT_FAILURE);
	}
	input.seekg(0);
	input.read(text_, n_);
  ...
  inline int isSmallerSuffix(const uint64_t i, const uint64_t j) {
	  return true;
  }
