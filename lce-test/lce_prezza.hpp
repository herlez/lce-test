...
class LcePrezza : public LceDataStructure {
	public:
		...
		LcePrezza(const std::string path) :
									...
									text_length_in_blocks_{text_length_in_bytes_/8 + (text_length_in_bytes_ % 8 == 0 ? 0 : 1)},
									prime_{util::getLow64BitPrime()},
									fingerprints_{new uint64_t[text_length_in_blocks_]},
									power_table_{new uint64_t[((int) std::log2(text_length_in_blocks_)) + 6]} {
			std::ifstream input(path, std::ios::in|std::ios::binary);
			util::inputErrorHandling(&input);
			input.seekg(0);
			input.read((char*)fingerprints_, text_length_in_bytes_);
			calculateFingerprints();
			calculatePowers();
		}
		
		/* Loads a prefix of the file located at PATH and build Prezza's LCE data structure */
		LcePrezza(const std::string path, const uint64_t number_of_chars) :
		...
		uint64_t lce(const uint64_t i, const uint64_t j) {
			...
			uint64_t comp_block_i = (block_i << offset_lce1) + ((block_i2 >> 1) >> (63-offset_lce1));
			uint64_t comp_block_j = (block_j << offset_lce2) + ((block_j2 >> 1) >> (63-offset_lce2));
			...
			while(lce < max_block_naive) {
				...
				comp_block_i = (block_i << offset_lce1) + ((block_i2 >> 1) >> (63-offset_lce1));
				comp_block_j = (block_j << offset_lce2) + ((block_j2 >> 1) >> (63-offset_lce2));
			}
	...
	private:
		...
		uint64_t getBlock(const uint64_t i) const {
			if (unlikely(i > text_length_in_blocks_)) {
				return 0;
			}
			if (unlikely(i == 0)) {
				...
				y = y <= current_fingerprint ? current_fingerprint - y : prime_ - (y - current_fingerprint);
		...
		uint64_t fingerprintExp(const uint64_t from, const int exp) const {
			if (unlikely(from == 0)) {
				...
				return fingerprint_to_j >= fingerprint_to_i ? (uint64_t) (fingerprint_to_j - fingerprint_to_i) : (uint64_t)  (prime_ - (fingerprint_to_i - fingerprint_to_j));
			}
		}
		...
		void calculatePowers() {
			unsigned int number_of_levels = ((int) std::log2(text_length_in_blocks_)) + 6; // +1 to round up and +4 because we need bit shifts by 1byte, 2byte, 4byte and 8byte
